...
class MoveGoal {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize the length for message field [cmd_seq]
    bufferOffset = _serializer.uint32(obj.cmd_seq.length, buffer, bufferOffset);
    obj.cmd_seq.forEach((val) => {
      bufferOffset = MoveCmd.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    // Deserialize array length for message field [cmd_seq]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.cmd_seq = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.cmd_seq[i] = MoveCmd.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    let length = 0;
    object.cmd_seq.forEach((val) => {
      length += MoveCmd.getMessageSize(val);
    });
    return length + 5;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.cmd_seq !== undefined) {
      resolved.cmd_seq = new Array(msg.cmd_seq.length);
      for (let i = 0; i < resolved.cmd_seq.length; ++i) {
        resolved.cmd_seq[i] = MoveCmd.Resolve(msg.cmd_seq[i]);
      }
    }
    else {
      resolved.cmd_seq = []
